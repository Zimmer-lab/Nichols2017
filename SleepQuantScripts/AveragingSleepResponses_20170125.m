...
saveDir = '/Volumes/groups/zimmer/Annika_Nichols/_Let_Assays/_5_ALS_ToAverage/_5.Figures/npr1_noStim';%'\\storage.imp.ac.at\groups\zimmer\Annika_Nichols\_5_ALS_ToAverage\_3.RESULTS\_20160531';
...
for i = 1 : size(walkDir,1)
    ...
    if walkDir(i).isdir == 1 && strcmp(walkDir(i).name, '.') == 0 ...
        ...
        DataRange = [2000 1600];   %define in sec the range that spans the first block you wish to be plotted
        % first element has to be >= BinWinSec!; second element is the width
        
        PlotAvgSelect_MassiveP_speed_plotIndivCurves_20160125_AN(OutputFileName, currFolder, 'sleep', BinWinSec, DatasetPointer, SBinTrcksSpdSize, BinTrcksO, BinTrcksLR, BinTrcksSR, wakestate, motionstate, BinTrcksOstate, BinTrcksLRstate, BinTrcksSRstate, SBinWinSec,DataRange);
        PlotAvgSelect_MassiveP_speed_plotIndivCurves_20160125_AN(OutputFileName, currFolder, 'wake', BinWinSec, DatasetPointer, SBinTrcksSpdSize, BinTrcksO, BinTrcksLR, BinTrcksSR, wakestate, motionstate, BinTrcksOstate, BinTrcksLRstate, BinTrcksSRstate, SBinWinSec,DataRange);
