...
function PlotAvgSelect_MassiveP_speed_plotIndivCurves_20160628_Bluelight(FilePrefix, PlotTitle, StateToPlot, BinWinSec, DatasetPointer, ...
    ...
    StimDist = 100; % distance between blocks (or stimuli) in seconds

    DataRange = [3000 3600];   %define in sec the range that spans the first block you wish to be plotted
                                % first element has to be >= BinWinSec!; second element is the width

    FirstStimulus = [3600 3]; %start and width of first stimulus

    SampleRate = 3; %framerate at which movie has been recorded

    pixelsize=0.0; %pixelsize in mm
    
    %%
    ConditionalBinSec = [3300 3540]; %[3180 3480];%[1560 1560+360]; time window in which to sort tracks into sleep vs wake
    ...
    [~,widthEx] = size(SBinTrcksSpdSize);
    %StResh = 1:widthEx;
    StResh = (1:widthEx)-FirstStimulus(1);


    %DataRange1 = floor([DataRange; DataRange(1)+StimDist DataRange(2)]/BinWinSec);
    DataRange1 = floor([DataRange]/BinWinSec);
    ...
    %added to avoid second stim... bad work around
    SelectData(:,2)= 0;
        ...
        for ii = 1:NumStim
            ...
            if SelectData(i,ii)==1
                SBinTrcksSpdSizeResh = [SBinTrcksSpdSizeResh; ...
                    SBinTrcksSpdSize];%(DatasetPointer(i,1):DatasetPointer(i,2), DataRange1(ii,1):DataRange1(ii,1))];
                BinTrcksOResh = [BinTrcksOResh; ...
                    BinTrcksO];%(DatasetPointer(i,1):DatasetPointer(i,2), DataRange1(ii,1):DataRange1(ii,1))];
                BinTrcksLRResh = [BinTrcksLRResh; ...
                    BinTrcksLR];%(DatasetPointer(i,1):DatasetPointer(i,2), DataRange1(ii,1):DataRange1(ii,1))];
                BinTrcksSRResh = [BinTrcksSRResh; ...
                    BinTrcksSR];%(DatasetPointer(i,1):DatasetPointer(i,2), DataRange1(ii,1):DataRange1(ii,1))];
                WakeStateResh = [WakeStateResh; ...
                    wakestate];%(DatasetPointer(i,1):DatasetPointer(i,2), DataRange1(ii,1):DataRange1(ii,1))];
                WakeMotionStateResh = [WakeMotionStateResh; ...
                    motionstate];%(DatasetPointer(i,1):DatasetPointer(i,2), DataRange1(ii,1):DataRange1(ii,1))]; %AN
                BinTrcksOstateResh = [BinTrcksOstateResh; ...
                    BinTrcksOstate];%(DatasetPointer(i,1):DatasetPointer(i,2), DataRange1(ii,1):DataRange1(ii,1))];
                BinTrcksLRstateResh = [BinTrcksLRstateResh; ...
                    BinTrcksLRstate];%(DatasetPointer(i,1):DatasetPointer(i,2), DataRange1(ii,1):DataRange1(ii,1))];
                BinTrcksSRstateResh = [BinTrcksSRstateResh; ...
                    BinTrcksSRstate];%(DatasetPointer(i,1):DatasetPointer(i,2), DataRange1(ii,1):DataRange1(ii,1))];
            end
        end
    end
