...
%vs:
%ActivityPlot!!!!
[SBinWinSec, SBinTrcksSpd, SBinTrcksSpdWght, BinTrcksEcc, BinTrcksEccWght, Sbintrcknum, St] = spdalsV5_AN_noRing(Tracks,binning);
...
PreCOrmdwlstate = NaN(NumTracks,NumBins);
PreCOPostureState = NaN(NumTracks,NumBins);
...
for idx = 1:NumTracks;
    ...
    for sldidx = 1+SlidingWinSizeBins/2 : NumBins-SlidingWinSizeBins/2
        ...
        if Tracks(idx).NumFrames > (SlidingWinSizeBins*BinSizeFrames)
            ...
            PreCOrmdwlstate(idx,sldidx) = nanmean(SBinTrcksSpdSize(idx,sldidx-SlidingWinSizeBins/2:sldidx+SlidingWinSizeBins/2));
            ...
            PreCOPostureState(idx,sldidx) = nanmean(DBinSmoothedEccentricityDSt(idx,sldidx-SlidingWinSizeBins/2:sldidx+SlidingWinSizeBins/2));
        end;
    end;
end;
