function [backgroundMax] = CreateBackgroundImageMax(basename,frameinterval)
...
for Frame = 1:frameinterval:FileInfo.NumberOfFrames
    cnt=cnt+1;
    currentImage = read(FileInfo, Frame);
    imageStack(:,:,cnt) = rgb2gray(currentImage);
    fprintf('%d \n',cnt);
end
...
cdatamax = max(imageStack,[],3);

backgroundMax = uint8(cdatamax);
% backgroundAve = uint8(round(cdataaverage*255));