function fi=wbMakeTMIPsFromImageSequence(mainFolder,smoothingWindow,targetFolder)
...
disp(['wbMakeTMIPsFromImageSequence> generating ' num2str(fi.numTW) ' TMIPs...']); 
tic
...
mkdir([parentFolder filesep mainFolderName 'TMIPs']);
...
for tw=1:fi.numTW
     ...
     for i=1:numberImages   
          ...
          for z=1:fi.numZ
              ...
              thisImage=double(imread( tifFileNames_sorted{frameCount} ));
...
%               end
     ...
     for z=1:fi.numZ
          ...
          imwrite(uint16(accumImage{z}),[thisZFolder{z} filesep 'TMIP-TW' num2str(smoothingWindow) '-Z' num2str(z,'%02d') '-T' num2str(tw,['%0' num2str(numMaxDigits) 'd']) '.tif'] ,'TIFF','Compression','LZW');
     end
