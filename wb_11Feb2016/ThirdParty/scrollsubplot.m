...
if ~isempty(scroll_hndl)
  %Saul Kato: add active listener
  hListener = addlistener(scroll_hndl,'Value','PostSet',@(scr,event) LiveScroll) ;
end
...
if isempty(scroll_hndl)
   scroll_hndl=uicontrol('Units','normalized',...
            'Style','Slider',...
            'Position',[.98,0,.02,1],...
            'Min',0,...
            'Max',1,...
...
%     function LiveScroll
%         get(scroll_hndl,'Value')
%         old_val_global
%         if old_val_global-get(scroll_hndl,'Value')>0.07
%             scroll(old_val_global);
%             disp('yo');
%             drawnow;
%             old_val_global=get(scroll_hndl,'Value')
%         end
%         
% 
%     end
   ...
   % LIVE SCROLL Callback

   newScrollValue=get(scroll_hndl,'value');
...
%    
   if newScrollValue ~= lastScrollValue
       ...
       ypos = (1-newScrollValue)*diff(pos_y_range);  
   ...
   lastScrollValue=get(scroll_hndl,'value');
