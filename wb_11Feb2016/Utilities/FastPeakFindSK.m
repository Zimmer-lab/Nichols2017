function  [federatedcenters, varargout]=FastPeakFindSK(d,threshold,filt,minspacing,med_filt_width,edg,fid,templateFilter,max_cutoff,post_thresh)
...
if any(d(:))  ; %for the case of non zero raw image
    ...
    if ~isempty(max_cutoff) && max_cutoff>0
             d(d>max_cutoff)=max_cutoff;
    end
    
    if ~isempty(med_filt_width) && med_filt_width>1
    ...
    if any(d(:))   ; %for the case of the image is still non zero
        ...
        if templateFilter==0
            ...
            [I_SSD,I_NCC]=template_matching(templateFilter,d);
            d=1000*(I_NCC-0.5);
...
if nargout>1 ;  varargout{1}=cent_map; varargout{2}=cent; varargout{3}=links; varargout{4}=templated_image;varargout{5}=filtered_image; end