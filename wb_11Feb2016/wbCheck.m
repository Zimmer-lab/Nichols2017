function wbCheck(neuron,wbstructFileName,passedHandles,loadMovieFlag)
%wbCheck(neuron,wbstructfile,passedHandles,loadMovieFlag)
%Saul Kato
%20131015
...
if ~loadMovieFlag
    disp('loadMovieFlag set to false.  Running in movieless mode');
end
...
if loadMovieFlag
    ...
    for i=1:metadata.fileInfo.numZ
        ZMovie{i}=zeros(metadata.fileInfo.width,metadata.fileInfo.height,metadata.fileInfo.numT);
    end
end
...
%         end
    ...
    function currentZPlane=getNewNeuronData()
        ...
        %load IDs
        if isfield(wbstruct,'ID')  && size(wbstruct.ID1,2)>=neuron
...
% 
    ...
    function drawPlots()
       ...
       if loadMovieFlag
           ...
           hold off;
           cropframe_full=ZMovie{currentZPlane}(:,:,frame);
...
%            
           ...
           if currentZPlane>1
               cropframe_full=ZMovie{currentZPlane-1}(:,:,frame);
           else
               cropframe_full=zeros(size(cropframe_full));
           end
           handles.lowerZimage=imagesc(cropframe_full,[clow chigh]);
...
%               end
            ...
            if isfield(wbstruct,'deltaFOverF_bc')
                ...
                wbstruct.deltaFOverF_bc(:,replacementNeuron)=wbProcessTraces(wbstruct.added.neighbors(end).deltaFOverF_bc(:,wbstruct.added.neighbors(end).picked),wbPToptions);
            end           
            ...
            if isfield(wbstruct,'deltaFOverF_bc')
                ...
                wbstruct.deltaFOverF_bc(:,wbstruct.nn+1)=bleachcorrect(wbstruct.added.neighbors(end).deltaFOverF(:,wbstruct.added.neighbors(end).picked),BCoptions);
            end
    ...
    function neuronID1PopupMenuCallback
         ...
         ID=wbstruct.ID;ID1=wbstruct.ID1;
    ...
    function neuronID2PopupMenuCallback
         ...
         ID=wbstruct.ID;ID2=wbstruct.ID2;
    ...
    function neuronID3PopupMenuCallback
         ...
         ID=wbstruct.ID;ID3=wbstruct.ID3;
