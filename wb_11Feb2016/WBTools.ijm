macro "Der Whole Brain Analyzer Action Tool - B1dCc00T0010WT9010BTe408O "  { 
// File.copy requires ImageJ 1.47j
print("\\Clear");  // clear log window
print("Running Whole Brain Overview Maker.");

print("Please open an OME Tiff.");
...
//TRYING TO EXTRACT DT FROM OMES
/*
Ext.setId(id);
...
//I don't think we need these for anything.
/*
//create MIP movie frames
print(">[3/6] Creating MIP movie frames.");
print(".");
File.makeDirectory(dir+"MIPs"); 

setBatchMode(true);
for (t=0; t<Tmax; t++) {
	//this needs to be changed if file naming is not T then Z
	run("Image Sequence...", "open=["+dirOriginals+"] number="+Zmax+" starting=1 increment=1 scale=100 file=T"+Tnum[t*Zmax]+" sort");
	makeRectangle(x,y,w,h);
	run("Crop");
	if (rotFlag==1) run("Rotate 90 Degrees Left");
	run("Z Project...", "start=1 projection=[Max Intensity]");
	//this needs to be changed if file naming is not T then Z
	saveAs("Tiff", dir+"MIPs/MIP_T"+Tnum[t*Zmax]); 
	close();
	close();
	if (t%10==0) print("\\Update:processing..."+t+"/"+Tmax);
}
print("\\Update:processed "+Tmax+"/"+Tmax+".");
setBatchMode(false);
*/

/*
print(">[3.5/5] Creating TMIPs.");
...
for (z=0;z<numZ;z++)
{
	for (t=0; t<numT; t=t+100) 
	{
		...
		saveAs("Tiff", destDir[z]+"/TMIP_Z"+paddedZ[z]+paddedT[t]+".tif");  //Z folder
		close();
	}
}
*/
...
/*
print(">[4/5] Creating Z-movies and ZMIPs.");

File.makeDirectory(dir+"ZMovies"); 
...
for (z=0; z<numZ; z++)
{
	...
	else
	{
		...
		saveAs("Tiff", dir+"ZMovies/Z"+paddedZ[z]+".tif");
    }
...
/*
//create Montage MIP
print(">[6/6] Making montages.");
print("processing...0/"+Tmax);
File.makeDirectory(dir+"Montages");
setBatchMode(true);
for (t=0; t<Tmax; t++) {
	//this needs to be changed if file naming is not T then Z
	run("Image Sequence...", "open=["+dir+"StabilizedFrames] number="+Zmax+" starting=1 increment=1 scale=100 file=T"+Tnum[t*Zmax]+" sort");
	run("Make Montage...", "columns="+Zmax+" rows=1 scale=1 increment=1 border=0");  //good for tall narrow

	//this needs to be changed if file naming is not T then Z
	saveAs("Tiff", dir+"Montages/Montage"+Tnum[t*Zmax]); 
	close();
	close();
	if (t%10==0) {
		print("\\Update:processing..."+t+"/"+Tmax);
	}
}
setBatchMode(false);
print("\\Update:processed "+Tmax+"/"+Tmax+".");
...
f = File.open(dir+"cropCoords.txt");
print(f,x+" "+y+" "+w+" "+h); 
File.close(f);
