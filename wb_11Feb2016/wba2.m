function wbstruct=wba2(folder,options,extraOptions)
%WBA Whole Brain Analyzer
%
%V4 load movies files progressively
...
chigh=20000;  %absolute brightness, not currently using
clow=500;
...
if ~wbCheckForDataFolder(folder) || imageSequenceModeFlag
    ...
    tifFiles=dir([folder filesep '*.tif']);
    ...
    %load one .tif file
    t=1;
    ZMovieOneFrame{1}(:,:)=imread( tifFileNames_sorted{t});
...
if (nargin<2 || isempty(options)) && exist([folder filesep 'wboptions.mat'],'file')==2
    ...
    options=wbcreatedefaultwboptionsfile('v2');
end
...
if ~isfield(options,'motionCorrectionFlag')  %first perform global motion correction
    options.motionCorrectionFlag=true;
end

if ~isfield(options,'motionCorrectionMethod')  %first perform global motion correction
    options.motionCorrectionMethod='translate';   %will implement 'rigid'
end

if ~isfield(options,'moCoMedFiltWidth')
    options.moCoMedFiltWidth=3;
end
...
if ~isfield(options,'RAMModeFlag')  %make movies along the way
    if ~isfield(extraOptions,'RAMModeFlag') 
       options.RAMModeFlag=true;
    else
       options.RAMModeFlag=extraOptions.RAMModeFlag;
    end
end

if ~isfield(options,'genResultPlots') %generate heat and grid plots when finished
    options.genResultPlots=true;
end
...
if ~isfield(options,'blobConnectFlag')  %for debugging purposes, skip blob detection section
    options.blobConnectFlag=false;
end
...
if ~isfield(options,'outputMovieFrameRate')  %for extra movie output
...
if isfield(metadata,'noseDirection') && (strcmp(metadata.noseDirection,'North') || strcmp(metadata.noseDirection,'South'))
...
if options.motionCorrectionFlag  && ~exist([folder filesep 'Quant' filesep 'wbmoco.mat'],'file')
    ...
    moCoDrift=wbMotionCorrection(folder,options.moCoTimeStep,options.moCoMedFiltWidth,options.motionCorrectionMethod,false);
...
if options.createTMIPsFlag || ~exist([folder '/TMIPs'],'dir')
    ...
    if imageSequenceModeFlag
      fi=wbMakeTMIPsFromImageSequence(folder,smoothingTWindow);
    else
...
%% Initialize mo co drift variables
...
if options.blobDetectFlag || evalin('base','~exist(''tblobs'',''var'')')
    ...
    for z=validZs
        ...
        for tw=1:numTW
            ...
            federatedcenters=FastPeakFindSK(frame,threshold,filt,options.minBlobSpacing,options.medFiltWidth,3,[],defaultTemplateFilter,options.maxCutoff,options.postThreshold);
    ...
    if options.makeMoviesFlag
        ...
        videoOutObj=wbSetupOutputMovie(movieOutName,options.outputMovieQuality,options.outputMovieFrameRate,[min([1.1*numZ*width options.maxPlotWidth]) 2.2*height]);
        ...
        for tw=1:numTW
            ...
            for z=validZs
                ...
                plot(tblobs(z,tw).x,tblobs(z,tw).y,'g+','MarkerSize',2,'LineStyle','none');
                plot(tblobs(z,tw).x,tblobs(z,tw).y,'go','MarkerSize',20,'LineStyle','none');
            ...
            intitle('new positions in green, previous frame positions in blue');
        ...
        clear videoOutObj;  %close BlobMovie
        toc
...
if options.blobConnectFlag || evalin('base','~exist(''blobThreads'',''var'')')
...
%           globalDriftY(z,tw)=nanmean(tblobs(z,tw).deltaY);        
        ...
        disp([num2str(orphanCount) ' orphans created.']);
...
if (z==1 && verboseFlag)
    ...
    if options.makeMoviesFlag
        ...
        videoOutObj=wbSetupOutputMovie(movieOutName,options.outputMovieQuality,options.outputMovieFrameRate,[ min([1.1*numZ*width options.maxPlotWidth]) 1.1*height]);
...
%                        if (blobThreads.x0(thisblobThread)-blobThreads_sorted.x0(b))^2+(blobThreads.y0(thisblobThread)-blobThreads_sorted.y0(b))^2<=Rmax2
    ...
    if options.makeMoviesFlag
        disp('wba> plotting blobThreads timelines.');
        tic
        warning('off','MATLAB:MKDIR:DirectoryExists');
...
if options.makeMoviesFlag
    ...
    videoOutObj=wbSetupOutputMovie(movieOutName,options.outputMovieQuality,options.outputMovieFrameRate,[min([1.2*numZ*width options.maxPlotWidth]) 1.2*height]);
...
if (options.quantFlag)
    ...
    if options.RAMModeFlag
        ...
        movieLength=size(ZMovie{1},3);
        
    else
        disp('wba2> Running in disk (RAM-efficient) mode.');
        ...
        xbound=size(ZMovieOneFrame{1},2);
        ybound=size(ZMovieOneFrame{1},1);
        movieLength=metadata.fileInfo.numT;
    ...
    f_parents=zeros(numZ,length(blobThreads.parentlist));
    f_bonded=zeros(numZ,length(blobThreads.parentlist));
    f_background=zeros(numZ,length(blobThreads.parentlist));
    ...
    if options.interpolateMaskMotionFlag    
        ...
        bT_xinterp=zeros(movieLength,length(blobThreads_sorted.z));
        bT_yinterp=zeros(movieLength,length(blobThreads_sorted.z));
        ...
        if numTW>1
            for bt=1:length(blobThreads_sorted.z)
                bT_xinterp(:,bt)=round(interp1(twCenterTime,blobThreads_sorted.x(:,bt),1:movieLength,'linear','extrap'));
                bT_yinterp(:,bt)=round(interp1(twCenterTime,blobThreads_sorted.y(:,bt),1:movieLength,'linear','extrap'));
            end
        else
            for bt=1:length(blobThreads_sorted.z)
        ...
        for frame=1:movieLength
           ...
           ZMovieOneFrame=GetZMovieFrame(frame);
...
%debug
%               blobThreads.parentlist(b)
%               size(blobThreads_sorted.z)
%               blobThreads_sorted.z(blobThreads.parentlist(b))
              
              cropframe=ZMovieOneFrame{blobThreads_sorted.z(blobThreads.parentlist(b))}(dataedge_y1:dataedge_y2,dataedge_x1:dataedge_x2);
              ...
              %background subtraction,just parent frame for now
               ...
               background.cropframe=ZMovieOneFrame{blobThreads_sorted.z(blobThreads.parentlist(b))}(background.dataedge_y1:background.dataedge_y2,background.dataedge_x1:background.dataedge_x2);   
              ...
              for bb=1:length(blobThreads_sorted.children{blobThreads.parentlist(b)}) %quantify multi  
                   ...
                   cropframechild=ZMovieOneFrame{blobThreads_sorted.z(blobThreads_sorted.children{blobThreads.parentlist(b)}(bb))}(dataedge_y1:dataedge_y2,dataedge_x1:dataedge_x2);
...
%               end
    ...
    if ~options.RAMModeFlag && ~imageSequenceModeFlag
        
       CloseTiffs(TifLinkRead,metadata);
   ...
   %wbstruct.ID=cell(1,length(wbstruct.f0)); %create blank ID cell array
    ...
    if isfield(wbstruct, 'blobThreads_sorted')
...
%     end       
    ...
    function ZMovieOneFrame=GetZMovieFrame(frameNum)
            ...
            ZMovieOneFrame=cell(numZ,1);  %initialize
