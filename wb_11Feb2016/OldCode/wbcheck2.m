function wbcheck2(neuron,wbstructfile,passedHandles)
%wbcheck2(neuron,wbstructfile,passedHandles)
%Saul Kato
%20131015
%view source movie for a neuron for wba2
...
%load reference motion (TMIP) movies
%refzmoviefiles=dir( [maindir '/SmoothStabZMovies/*.tif']);
if options.globalMovieFlag && evalin('base','exist(''RefZMovie'',''var'')==1');
    disp('>using RefZMovie already in workspace.');
    RefZMovie=evalin('base', 'ZMovie');
else       
    disp('>loading RefZMovie...'); 
    tic
    warning('off','MATLAB:imagesci:tiffmexutils:libtiffWarning');
    for z=1:length(refzmoviefiles)
        fprintf('%d...',z-1); 
        FileTif=[maindir '/SmoothStabZMovies/' refzmoviefiles(z).name];
        InfoImage=imfinfo(FileTif);
        mImage=InfoImage(1).Width;
        nImage=InfoImage(1).Height;
        NumberImages=length(InfoImage);
        RefZMovie{z}=zeros(nImage,mImage,NumberImages,'uint16');
        TifLink = Tiff(FileTif, 'r');
        for i=1:NumberImages   
            TifLink.setDirectory(i);   
            RefZMovie{z}(:,:,i)=TifLink.read();
        end
        TifLink.close();
    end
    fprintf('%d.\n',z);
    warning('on','MATLAB:imagesci:tiffmexutils:libtiffWarning');
    if options.globalMovieFlag
        assignin('base','RefZMovie',RefZMovie);
    end
    toc
end
numZ=length(zmoviefiles);
...
% 
       ...
       main.cropframe=ZMovie{parent_zplane}(dataedge_y1:dataedge_y2,dataedge_x1:dataedge_x2,frame);
       ...
       ref.cropframe=RefZMovie{parent_zplane}(background.dataedge_y1:background.dataedge_y2,background.dataedge_x1:background.dataedge_x2,tSlice);
      ...
      for bb=1:length(wbstruct.blobThreads_sorted.children{bp}) %quantify multi  
           ...
           cropframechild=ZMovie{wbstruct.blobThreads_sorted.z(wbstruct.blobThreads_sorted.children{bp}(bb))}(dataedge_y1:dataedge_y2,dataedge_x1:dataedge_x2,frame);
      ...
      f_bonded_one(frame)=mean(vals(1:wbstruct.options.numPixelsBonded));
    ...
    function drawPlots()
       ...
       hold off;
       cropframe_full=ZMovie{wbstruct.blobThreads_sorted.z(bp)}(:,:,frame);
       ...
       handles.parentPlaneBackgroundCircle=rectangle('Curvature',[1 1],'Position',[background.dataedge_x1 background.dataedge_y1 1+2*Rbackground 1+2*Rbackground],'EdgeColor',[1 0 0]);
    ...
    function updatePlots()
       ...
       if wbstruct.blobThreads_sorted.z(bp)<wbstruct.numZ
...
%% CALLBACKS
    ...
    function excludeButtonCallback
        ...
        wbgridplot2(wbstruct,wbgridplotOptions);
