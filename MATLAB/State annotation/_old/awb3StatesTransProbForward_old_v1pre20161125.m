...
ForwardPeriods = NaN(sum(~isnan(allFtransLength)),400);
count= 1;
...
hold on
[nelements,xcenters] = hist(fakeVector,startS(1,1:end-1));
bar(xcenters,nelements*(0.12/(max(n)+6)),'histc')
hold on  
plot(startS(1,1:end-1),FQprob)
hold on
plot(startS(1,1:end-1),FRprob,'r')
hold on;
scatter(startS(1,1:end-1),FQprob,12,'filled','b')
hold on;
scatter(startS(1,1:end-1),FRprob,12,'filled','r')

if logon
    hold on;
    axes1 = plotyy(startS(1,1:end-1),FQprob,startS(1,1:end-1),n,'semilogx');
...
% %a2 = axes1('YAxisLocation', 'Right')
    ...
    %%Prelet
    %set(ha(1),'ylim',[0 0.45],'ytick',[0,0.1,0.2,0.3,0.4,],'FontSize',14, 'YColor', 'k'); 
...
set(axes1(1),'xlim',[1,150],'xtick',[10^0,10^1,10^2,10^3],'FontSize',14);
set(axes1(2),'xlim',[1,150],'xtick',[10^0,10^1,10^2,10^3],'FontSize',14);
