...
clearvars recNum FtransNum stats BW forwardState
...
% figure; hist(selectFtransLength,5);
% hold on; plot(xvals,nelements); %selected forward periods lengths
% hold on; plot(TimeBins(1:(end-1)),FRtrans,'r');
% hold on; plot(TimeBins(1:(end-1)),FQtrans,'g');
% title('Histogram of selected Forward period lengths')
% ylabel('event number')
% %
% % figure; plot(nansum(ForwardPeriods'))
% % xlim([0,500])
%
%
% figure;
% set(gca,'fontsize',18)
% hist((nansum(ForwardPeriods')/5),5:10:165)
...
if logon
    ...
    ylim([0,62]); xlim([min(log10(TimeBins(1:(end-1))/5))-0.5,max(log10(TimeBins(1:(end-1))/5))+0.5])
