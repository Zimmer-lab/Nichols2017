...
%%                            
options.version.awbStateTransNeuQA = 'v3_20161020'; 
...
presets = {'ThresholdDistance','LowTime','HighTime'}; %'TrigNeuronPolarity','priorQuiesceFlag','priorQuiesceSec'
...
totalRecordingFrames = length(wbstruct.simple.deltaFOverF);
for iiii = 1:length(AllAtoQTransitionFrames);
    ...
    if RangeStart >0 && RangeEnd < (totalRecordingFrames+1)
...
if isfield(StateTrans,'ClosestRise_Act2Qui');
    ...
    StateTrans.AtoQtransition = AtoQtransition/wbstruct.fps;
end
if isfield(StateTrans,'ClosestRise_Qui2Act');
    ...
    StateTrans.QtoAtransition = QtoAtransition/wbstruct.fps;
end
