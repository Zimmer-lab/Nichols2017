...
refTransitionLineColor=color('lr');
...
if WBSTRUCT_MODE
    ...
    handles.datasetPopup=uicontrol('Style','popupmenu','String',dataFolderList,'Position',[200 780 200 15],'Value',find(strcmp(upper(dataFolderList),upper(currentDataFolder))),'Callback',@(s,e) DatasetPopupCallback);
end
...
if isempty(options.pairedRefTraces)
    ...
    handles.refNeuronCheckbox= uicontrol('Style','checkbox','Value',1,'Units','normalized','Position',[px-.005 py-.045 0.02 0.05],'Callback',@(s,e) RefNeuronCheckboxCallback);
    ...
    handles.thirdNeuronCheckbox= uicontrol('Style','checkbox','Value',0,'Units','normalized','Position',[px-.005 py-.115 0.02 0.05],'Callback',@(s,e) ThirdNeuronCheckboxCallback);
...
%% subs
    ...
    function LiveUpdatePlot
        ...
        wbstruct.simple.derivs.thisTraceColoring = thisTraceColoring;

        %select transitionType
        if get(handles.mainNeuronTransitionTypePopup,'Value')==1
            
            mainTransitionLineColor=color('r');
            transitionTypes='AllRises';
            
        elseif get(handles.mainNeuronTransitionTypePopup,'Value')==2
            
            mainTransitionLineColor=color('y');
            transitionTypes='AllFalls';
            
        else
            
            mainTransitionLineColor=color('gray');
    ...
    function DrawPlot
        ...
        if isempty(options.pairedRefTraces)
            ...
            handles.pairedRefTrace=plot(tv,options.pairedRefTraces(:,n),'Color',color('lr'));
            hold on; 
...
%% callbacks
    ...
    function recomputeDerivsCallback
        ...
        if ~WBSTRUCT_MODE            
            ...
            save(options.stateParamStructFile,'stateParams','regAlpha','thisTraceColoring');
    ...
    function recomputeAllDerivsCallback
        ...
        wbstruct.simple.derivs.traces=derivReg(traces,regAlpha);
        derivtraces=wbstruct.simple.derivs.traces;
        thisTraceColoring = wbstruct.simple.derivs.thisTraceColoring;
        DrawPlot;
        ...
        if ~WBSTRUCT_MODE
            ...
            save(options.stateParamStructFile,'stateParams','regAlpha','derivtraces','thisTraceColoring');
    ...
    function RefNeuronPopupCallback
        ...
        if n_ref>0
            ...
            elseif get(handles.refNeuronTransitionTypePopup,'Value')==2
                ...
                refTransitionLineColor=color('ly');
                refTransitionTypes='AllFalls';

            else

                refTransitionLineColor=color('lightgray');
