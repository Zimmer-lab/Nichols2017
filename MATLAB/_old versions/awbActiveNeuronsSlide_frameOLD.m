...
binHalf = floor((options.BinSize*wbstruct.fps)/2);

for frame = 1:frameTotal
    %set up range for bin for this frame
    frameStart = frame - binHalf;
    frameEnd = frame + binHalf;
...
IndividualActiveNeurons(1,frame) = sum(Active)/neuronNum;

%for single neurons
SingleActiveNeurons (frame,:)= Active (1,:);
