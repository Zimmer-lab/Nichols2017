...
function PlotAvgSelect_splitQA_Turn_CumS_FirstT(FilePrefix, PlotTitle, StateToPlot, BinWinSec, DatasetPointer, ...
                                                ...
                                                AllTurnStartsBinned,InstMotionState)
    %%
    ...
    % [1260 1500].
    ConditionalBinQASec = [1550 1560]; % time window in which to sort lethargic tracks into quiescent or active
    ...
    for i = 1:NumExp
        ...
        for ii = 1:NumStim
            ...
            if SelectData(i,ii)==1
                ...
                InstMotionStateResh = [InstMotionStateResh; ...
                    InstMotionState(DatasetPointer(i,1):DatasetPointer(i,2), DataRange1(ii,1):DataRange1(ii,1)+DataRange1(ii,2))]; %AN
    ...
    FollowingInstMotionStateSleep = NaN(NumTracksResh,NumBinsResh); %AN
    ...
    ConditionalBinQASec2 = [ConditionalBinQASec(1)+SBinWinSec,ConditionalBinQASec(2)];
    ConditionalBinQAFr = floor((ConditionalBinQASec2-DataRange(1)) / SBinWinSec); %corrects for taking one extra bin
    ...
    for i = 1:NumTracksResh
        ...
        if sum(WakeStateResh(i,ConditionalBinFr(1):ConditionalBinFr(2)))==0
            ...
            FollowingInstMotionStateSleep(i,:) = InstMotionStateResh(i,:); %AN
            ...
            if sum(InstMotionStateResh(i,ConditionalBinQAFr(1):ConditionalBinQAFr(2)))==0
                SleepTracksQuiescent(i,1) = countSleepTracks;
                
            elseif sum(InstMotionStateResh(i,ConditionalBinQAFr(1):ConditionalBinQAFr(2)))==(1+ConditionalBinQAFr(2)-ConditionalBinQAFr(1));
...
%     title('motionstate - Quiescent')
%     figure; imagesc(viewingFollowingMS(SleepTracksActive,:))
%     line([ConditionalBinQAFr(1),ConditionalBinQAFr(1)],[0,800])
%     line([ConditionalBinQAFr(2),ConditionalBinQAFr(2)],[0,800])
%     title('motionstate - Active')

    viewingFollowingMS=FollowingMotionStateSleep;
    viewingFollowingMS(isnan(nanmean(viewingFollowingMS')),:)=[];
    viewingFollowingMS(isnan(viewingFollowingMS))=3;
    figure; imagesc(viewingFollowingMS(:,:))
    line([ConditionalBinQAFr(1),ConditionalBinQAFr(1)],[0,800])
    line([ConditionalBinQAFr(2),ConditionalBinQAFr(2)],[0,800])
    title('instaneous motionstate')
    
    viewingFollowingMS=FollowingInstMotionStateSleep;
    viewingFollowingMS(isnan(viewingFollowingMS))=3;
    figure; imagesc(viewingFollowingMS(SleepTracksQuiescent,:))
    line([ConditionalBinQAFr(1),ConditionalBinQAFr(1)],[0,800])
    line([ConditionalBinQAFr(2),ConditionalBinQAFr(2)],[0,800])
    title('instaneous motionstate - Quiescent')
    figure; imagesc(viewingFollowingMS(SleepTracksActive,:))
    line([ConditionalBinQAFr(1),ConditionalBinQAFr(1)],[0,800])
    line([ConditionalBinQAFr(2),ConditionalBinQAFr(2)],[0,800])
    title('instaneous motionstate - Active')
...
%             stBinTurnSleep = nansterr(cumsum(BinTurnSleep,2));
            ...
            %timeplot=StResh((find(StResh == 0)):length(StResh));
            
            TurnFigure=Ofig(Oxmax,Tymax,stimuli);
            xlim([timeplot(1) timeplot(end)-400]);
        ...
        %% First Turn Binned
        StimStartBin = find(StResh == 0);

        firstRepsonseM = TurnSleep(:,StimStartBin:(StimStartBin+(FirstStimulus(2)/SBinWinSec)));
        FirstResponseBin = NaN(NumTracksResh,1);
        
        for TrackN =1:NumTracksResh
...
%         xcenters = 1.5:3:85;
        ...
        HistFTRQ = (hist(FirstResponseBin(SleepTracksQuiescent,1),xcenters))/length(SleepTracksQuiescent);
        HistFTRA = (hist(FirstResponseBin(SleepTracksActive,1),xcenters))/length(SleepTracksActive);
        figure; plot(xcentersSec,HistFTRQ); hold on; plot(xcentersSec,HistFTRA,'r')
        ylabel('fraction');
        xlabel('time from stimulus (s)');
        
        FractionTurningInFirstPeriod = [HistFTRQ(1,1); HistFTRA(1,1)];
        %need to find for each experiment...


        %% Motionstate QA
        mnFollowingMotionStateSleep = nanmean(FollowingInstMotionStateSleep(SleepTracksQuiescent,:));
        mnFollowingMotionStateSleepA = nanmean(FollowingInstMotionStateSleep(SleepTracksActive,:));
        FollowStateFig = DataFig(FurtherBinTime(1), FurtherBinTime(end), -0.05, 1.05, stimuli(1:2));
        plot(StResh, mnFollowingMotionStateSleep, 'b', 'linewidth', 2.5);
        hold on; plot(StResh, mnFollowingMotionStateSleepA, 'r', 'linewidth', 2.5);
        ylabel('fraction active');
        xlabel('time [s]');
        titleStr = sprintf('%s_%s_Fraction active_instMotionState_QA',PlotTitle,StateToPlot);
        title(titleStr, 'Interpreter', 'none');
        print (gcf, '-depsc', '-r300', sprintf('%s_%s_1-Fraction exiting quiescence_isntMotionState_QA.ai',FilePrefix,StateToPlot));
    ...
    if  strcmp(StateToPlot,'wake')
        ...
        %% Plotting cumsum of turns from stim onset.
            TurnSleepCumSumT = TurnWake(:,(find(StResh == 0)):length(StResh));
            ...
            TurnFigure=Ofig(Oxmax,Tymax,stimuli);
            xlim([timeplot(1) timeplot(end)-400]);
            title('Turning Active CumS');
            jbfill(timeplot,BinTurnSleepPlot+stBinTurnSleepPlot,BinTurnSleepPlot-stBinTurnSleepPlot,grey,grey,0,1);
            hold on; plot(timeplot,BinTurnSleepPlot,'b','linewidth',2.5);
            titleStr = sprintf('%s_%s_Turning Lethargus CumS',PlotTitle,StateToPlot);
            title(titleStr,'Interpreter','none');
            print (gcf, '-depsc', '-r300', sprintf('%s_%s_4-Turning Active CumS.ai',FilePrefix,StateToPlot));
            print (gcf, '-dpdf', '-r300', sprintf('%s_%s_4-Turning Active CumS.pdf',FilePrefix,StateToPlot));
