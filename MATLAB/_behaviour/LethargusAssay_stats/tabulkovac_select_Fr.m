function tabulkovac_select_Fr(dirPath)
%-example run-:tabulkovac_select_Fr('/Users/eichler/Desktop/__Averaging/___Let+PostLet_20130920_TestStats_Results/_Stats_select/2.N2+BAG- (CO2)');
clc
files    = dir(dirPath);
fid      = -1;
fileName = '';
    ...
    for i = 1 : length(files)
        ...
        if (files(i).isdir) == false
            ...
            if strcmp( fileExt,'.mat') == true
                ...
                switch(round)
                    case 1
                        var = 'FractionActive1SleepSelect';
                        [fid,fileOpened] = writeTableAsCSV(fid,fileOpened,fileName,var);
                    case 2
                        var = 'FractionActive1WakeSelect';
                        [fid,fileOpened] = writeTableAsCSV(fid,fileOpened,fileName,var);
                    case 3
                        var = 'FractionActive2SleepSelect';
                        [fid,fileOpened] = writeTableAsCSV(fid,fileOpened,fileName,var);
                    case 4
                        var = 'FractionActive2WakeSelect';
                        [fid,fileOpened] = writeTableAsCSV(fid,fileOpened,fileName,var);
                    case 5
