...
options.version = 'pharynxTracker_convex_v3';
...
% Notes on v3.
% Now properly gave NaNs to when there was no shape to track.
...
for Frame = 1:FileInfo.NumberOfFrames;
    ...
    if NUM > 0; %in case there is no centroid.
        ...
        %See if object satisfies both the min area size and min distance from
        %largest centroid.
        ...
        if NUM > 1;
            ...
            if ~isempty(ConHullPoints); %if there is not trackable area
                ...
                Centroid = [NaN,NaN];
                disp(strcat('No trackable area at frame:',mat2str(Frame)));
            end
        else
...
%             scatter(Centroid(1,1),Centroid (1,2), 'g','filled');
        ...
        options.lostFrames = [options.lostFrames, Frame];
    end
end
