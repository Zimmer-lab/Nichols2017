...
for dirIdx = 1 : size(walkDir,1)
    if walkDir(dirIdx).isdir == 1 && strcmp(walkDir(dirIdx).name, '.') == 0 && strcmp(walkDir(dirIdx).name, '..') == 0
        ...
        clearvars -except AllSpeed AllEccen aAllMotionState aAllWakeState mainDir saveDir walkDir dirIdx SaveQuantDataFilename currFolder AllWakeState AllMotionState;
        %close all;
...
%%
%save (([strcat(mainDir,'/DishWakeState') '.mat']), 'AllMotionState','AllSpeed','AllWakeState', 'aAllWakeState','aAllMotionState'); 
