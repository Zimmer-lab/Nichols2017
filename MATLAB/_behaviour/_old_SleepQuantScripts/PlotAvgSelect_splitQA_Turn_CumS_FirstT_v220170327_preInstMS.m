...
function PlotAvgSelect_splitQA_Turn_CumS_FirstT(FilePrefix, PlotTitle, StateToPlot, BinWinSec, DatasetPointer, ...
    ...
    % [1260 1500].
    ConditionalBinQASec = [1525 1560]; % time window in which to sort lethargic tracks into quiescent or active
    ConditionalBinQASec = [1540 1560]; % time window in which to sort lethargic tracks into quiescent or active
...
%         xcenters = 1.5:3:85;
        ...
        %% Motionstate QA
        mnFollowingMotionStateSleep = nanmean(FollowingMotionStateSleep(SleepTracksQuiescent,:));
        mnFollowingMotionStateSleepA = nanmean(FollowingMotionStateSleep(SleepTracksActive,:));
        FollowStateFig = DataFig(FurtherBinTime(1), FurtherBinTime(end), -0.05, 1.05, stimuli(1:2));
        plot(StResh, mnFollowingMotionStateSleep, 'b', 'linewidth', 2.5);
        hold on; plot(StResh, mnFollowingMotionStateSleepA, 'r', 'linewidth', 2.5);
        ylabel('fraction active');
        xlabel('time [s]');
        titleStr = sprintf('%s_%s_Fraction active_MotionState_QA',PlotTitle,StateToPlot);
        title(titleStr, 'Interpreter', 'none');
        print (gcf, '-depsc', '-r300', sprintf('%s_%s_1-Fraction exiting quiescence_MotionState_QA.ai',FilePrefix,StateToPlot));
