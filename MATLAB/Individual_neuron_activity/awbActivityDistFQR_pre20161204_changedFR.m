...
%Neurons ={'RIS','RMED','RMEV'};

ResultsStructFilename = 'ActivityDistFQR';
...
for recNum = 1:NumDataSets %Folder loop
    ...
    for neuNum = 1:length(Neurons)
        ...
        if ~isnan(Trace)
            ...
            ForTrace = Trace(RangeForward);
            RevTrace = Trace(RangeReversal);
...
%subplot:
fig = figure;
for neuNum = 1:48;%length(Neurons)
    subplot(6,8,neuNum)
    if neuNum >= 24
        neuNum =neuNum+1;
    end
...
%for neuNum = 1:length(Neurons)
    ...
    if saveflag == 1;
        ...
        print (gcf,'-depsc', '-r300', sprintf([inputData{neuNum},'_ActDistFQR.ai']));
    end
end
...
if SubplotSaveflag == 1;
    ...
    set(gcf,'PaperPositionMode','auto')
    print (gcf,'-depsc', '-r300', sprintf('npr1PreLet_ActDistFQR.ai'));
end
%% find distances
