...
for recNum = 1:NumDataSets %Folder loop
    ...
    if State2 == 'F'
        fullRange2States(RangeReversal) = [];
    elseif State2 == 'R'
        fullRange2States(RangeForward) = [];
    else
    ...
    for neuNum = 1:length(Neurons)
        ...
        if ~isnan(TraceAll.(Neurons{neuNum}))
            ...
            if State2 == 'F'
                Trace.(Neurons{neuNum})(RangeReversal) = [];
            elseif State2 == 'R'
                Trace.(Neurons{neuNum})(RangeForward) = [];
            else
                disp('Confused about State2 input!')
                return
            end
...
%save ('/Users/nichols/Dropbox/_Analysing sets/npr1_2_Let/ResamplingActivityDistFQnpr1LetLog20161104.mat', 'trueDistances','sampleDistances','Neurons','xcentres','resamplingNum','pValue','-v7.3');
