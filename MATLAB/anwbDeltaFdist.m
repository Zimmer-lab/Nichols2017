...
load('Quant/wbstruct.mat','simple');
      ...
      NeuronATraces = deriv(simple.deltaFOverF);
      
      NeuronATraces = abs(NeuronATraces);
...
BinVec = logspace(-0,0.05,20); %log

BinVec(:)=BinVec(:)-1

N =[];

[NumAllNeurons, ~] = size(NeuronATraces);

for iii = 1:NumBins
    

    
N(iii,:) = hist(NeuronATraces(:,iii),BinVec); %/NumAllNeurons;
...
figure; plot(mean(NeuronATraces));
...
figure;
             plot(mean(NeuronATraces), 'k');
             hold on
             plot(min(NeuronATraces), 'color', [0.4 0.4 0.4]);
             hold on
             plot(max(NeuronATraces), 'color', [0.4 0.4 0.4]);
