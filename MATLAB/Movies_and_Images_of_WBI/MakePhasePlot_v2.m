...
%extra for colouring the dorsal/ventral turns
VentralTurn=[1,1,1,1,1,0]; %1112b let
stats = regionprops(ReversalFALL,'BoundingBox');
[numFalls, ~]= size(stats);
if numFalls ~= length(VentralTurn);
    disp('false number of falls annotated!')
end

FallVentral = zeros(size(ReversalRISE));
FallDorsal = zeros(size(ReversalRISE));
for fall = 1:numFalls;
    fallStart = stats(fall, 1).BoundingBox(2) + 0.5;
    if VentralTurn(fall)
        FallVentral(fallStart:(fallStart+stats(fall, 1).BoundingBox(4) -1),1) = 1;
    else
        FallDorsal(fallStart:(fallStart+stats(fall, 1).BoundingBox(4) -1),1) = 1;
    end
end
%figure; imagesc([FallVentral,FallDorsal,ReversalFALL])
...
CventralTurn = [50 205 50]/255;

recessmap = [Cforward;Cquiescence;Creversal;Cturn;CventralTurn;grey];
...
colorm(find(ReversalHIGH),1) = 2; 
%colorm(find(ReversalFALL)) = 3;
colorm(find(FallDorsal)) = 3; 
colorm(find(FallVentral)) = 4;
