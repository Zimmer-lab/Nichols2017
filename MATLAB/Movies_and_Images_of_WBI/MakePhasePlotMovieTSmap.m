...
FolderName = 'PhasePlotMovie_3rdFrame_TSmap'; 
...
sevenStateColoring.keyNew = {'Forward  ','Slow  ','Dorsal Turn  ','Ventral Turn  ',...
    'Reversal1   ','Reversal2   ','Sustained Reversal    ','No state  '};
...
% Derivative
tpc1 = (pcs(:,1)); %pcsFullRange
tpc2 = (pcs(:,2)); %pcsFullRange
tpc3 = (pcs(:,3)); %pcsFullRange
...
%     subimage(subplot3, jet)
    ...
    dummyColorVector =(ones(length(sevenStateColoring.dataset(1, 1).stateTimeSeries),1));
    
    color_line3(tpc1,tpc2,tpc3,(dummyColorVector+7),'LineWidth',1.3);
%     color_line3(tpc1,tpc2,tpc3,(ones(length(tpc1),1)),'LineWidth',2.5);
%     colormap([0,0,0]);
    
    %xlabel('PC1');ylabel('PC2');zlabel('PC3');
    colormap(sevenStateColoring.colorMap);
    ...
    %MAY NEED TO CHANGE!
    %ylim([-1,1]) %Pre 30a
    xlim([-0.23,0.28]) %Let 31a
    
    %set(gca, 'PlotBoxAspectRatio',[1 1 1])
    grid on;
    
    %darker colouring
    largerT = Frame-50:Frame+50; %50
    if sum(largerT == 0)
    ...
    hold on;
    color_line3(tpc1(largerT,1),tpc2(largerT,1),tpc3(largerT,1),dummyColorVector(largerT,1),'LineWidth',5);
    caxis([1,8])
    
    %ball
    hold on;
    scatter3(tpc1(Frame,1),tpc2(Frame,1),tpc3(Frame,1),170,'k','fill');
    
    %State colour labeling %MAY NEED TO CHANGE!
    %text(0.44,0.20,sevenStateColoring.keyNew{(sevenStateColoring.dataset(1, 1).stateTimeSeries(Frame))},'Fontsize',16,'Color',sevenStateColoring.colorMap((sevenStateColoring.dataset(1, 1).stateTimeSeries(Frame)),:),'FontWeight','bold');
    %31a 3.2, 1 %TS 4,3.5
    
    %Plot seconds and oxygen
    hold on
    %weird changes position in printed version
    text(0.23,0.33,strcat('Time: ', num2str(seconds),'s'),'Fontsize',16,'Color','k'); %31a 1.3,2.5 %TS 2,6
%     if seconds < 360
%         oxCon = 'Oxygen: 10%';
%     elseif seconds > 720
%         oxCon = 'Oxygen: 10%';
...
%     end
%     
%     text(0.21,0.315,oxCon,'Fontsize',16,'Color','k'); %31a 1.1,2.3 %TS 1.8,5.7
