...
for Frame = 1:3:60;%NumFrames; %total recording    
    % Create figure
    figure1 = figure;
    hold on;
...
%     hold off
    ...
    imagesc(fliplr(flipud(imageStack(1:450,:,Frame)')));
    ...
    hold on; 
    h= text(490,50,'Fluorescence (a.u.)  ','Fontsize',16,'Color','k');
    set(h, 'rotation', 90)
    ...
    % Plot scale bar 10um
    hold on;
    line('XData', [0, (10+((pixelsize)*10))], 'YData', [150, 150],'color','k','LineStyle', '-','Linewidth',8);
    text(0,155,'10um','Fontsize',16,'Color','k')
    
    %Plot seconds and oxygen
    hold on
    text(0,-20,strcat(num2str(seconds),'s'),'Fontsize',16,'Color','k');
    if seconds < 360
        oxCon = 'Oxygen: 10%';
    elseif seconds > 720
        oxCon = 'Oxygen: 10%';
    ...
    text(0,-10,oxCon,'Fontsize',16,'Color','k');
    
    subimage(subplot3, jet)

%     % Plot phase plot
%     subplot1 = subplot(2,4,[1,2,5,6],'Parent',figure1,'PlotBoxAspectRatio',[1 1 1]);
...
%     hold(subplot1,'all');
%     
%     color_line3(tpc1,tpc2,tpc3,grayColorm,'LineWidth',1);
...
%     
%     hold on;
%     color_line3(tpc1(largerT,1),tpc2(largerT,1),tpc3(largerT,1),colorm(largerT,1),'LineWidth',2);
%     caxis([0,5])
%     
%     %ball
%     hold on;
...
%     text(3.2,1,stateName{(colorm(Frame)+1)},'Fontsize',16,'Color',recessmap((colorm(Frame)+1),:),'FontWeight','bold');
