...
binHalfS = ((options.BinSize)/2);
binHalfF = ((options.BinSize*wbstruct.fps)/2);

for second = 1:(totalRecordingLength-binHalfS)
    %set up range for bin for this frame
    frameStart = ceil((second*wbstruct.fps) - binHalfF);
    frameEnd = floor((second*wbstruct.fps) + binHalfF);
    
    %Accounting for edged bins %%%%%%%FIX TO NOT INCLUDE EDGES
    if frameStart > 0
