function [traces,traceSimpleIndices,tv,trialnames,f0]=wbGetTracesAN(wbstructOrRootFolder,labeledOnlyFlag,fieldName,neuronSubset)
...
if isstruct(wbstructOrRootFolder)
    wbstruct2=wbstructOrRootFolder;
else
    wbstruct2=wbload(wbstructOrRootFolder,false);
end

if isstruct(wbstruct2)   %not a cell array
    wbstruct2={wbstruct2};
end
...
for i=1:length(wbstruct2)
    
    trialnames{i}=wbstruct2{i}.trialname;

    if isfield(wbstruct2{i},'simple') && strcmp(fieldName,'deltaFOverF')

        traces{i}=wbstruct2{i}.simple.deltaFOverF;
        tv{i}=wbstruct2{i}.simple.tv;
        
        if isfield(wbstruct2{i}.simple,'f0')
            f0{i}=wbstruct2{i}.simple.f0;
        end
        
    else

        traces{i}=wbstruct2{i}.(fieldName);
        tv{i}=wbstruct2{i}.tv;
        traces{i}(:,wbstruct2{i}.exclusionList)=[];
        if isfield(wbstruct2{i},'f0')
            f0{i}=wbstruct2{i}.f0;
        end
    end
    ...
    if ~isempty(neuronSubset)
        ...
        elseif iscell(neuronSubset)
            ...
            for j=1:length(neuronSubset)
                ...
                [thisTrace, ~, thisSimpleIndex,thisF0] = wbgettraceAN(neuronSubset{j},wbstruct2{i},fieldName);
...
%         end    
        ...
        traceSimpleIndices{i}=find(~cellfun(@isempty,wbstruct2{i}.simple.ID1));
        traces{i}=traces{i}(:,~cellfun(@isempty,wbstruct2{i}.simple.ID1));
        f0{i}=f0{i}(~cellfun(@isempty,wbstruct2{i}.simple.ID1));
